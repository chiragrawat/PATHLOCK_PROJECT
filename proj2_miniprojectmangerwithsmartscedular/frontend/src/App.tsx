    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          throw new Error(
            errData.title || errData.message || `HTTP error! status: ${response.status}`
          );
        } catch {
...
const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const handleAuthError = () => {
      console.log('Auth error (401) detected. Logging out.');
      setToken(null);
      setEmail(null);
    };
...
const Router = () => {
  ...
  useEffect(() => {
    ...
    if (isAuthPage && auth.isAuthenticated) navigate('/dashboard');
    else if (!isAuthPage && !auth.isAuthenticated) navigate('/auth');
  }, [page, auth.isAuthenticated, auth.isLoading]);

  if (auth.isLoading) return <LoadingSpinner text="Loading session..." />;
...
const ErrorDisplay = ({ message }: { message: string | null }) => {
  ...
  return (
    <div className="bg-red-900 border border-red-700 text-red-100 px-4 py-3 rounded-lg my-4">
...
const inputStyle =
  'w-full px-4 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-transparent';
const btnStyle =
  'bg-indigo-600 hover:bg-indigo-700 text-white font-medium py-2 px-4 rounded-lg shadow-md transition duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-opacity-75';
...
const ProjectDetailPage = ({
  ...
  if (isLoading) return <LoadingSpinner text="Loading project details..." />;
  if (error && !project) return <ErrorDisplay message={error} />;
  if (!project) return <p className="text-gray-400">Project not found.</p>;

  return (
    <div className="space-y-6">
      <button onClick={() => onNavigate('/dashboard')} className="text-indigo-400 hover:text-indigo-300 text-sm">
      ...
      {project.description && <p className="text-gray-300 text-lg">{project.description}</p>}
      ...
      <form onSubmit={handleAddTask} className="space-y-4 md:space-y-0 md:flex md:gap-4">
        ...
        <button
          type="submit"
          className={`${btnStyle} w-full md:w-auto flex items-center justify-center gap-2`}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  {task.dueDate && (
                    <span
                      className={`text-xs ${
                        task.isCompleted ? 'text-gray-600 line-through' : 'text-indigo-300'
                      }`}
                    >
      ...
      {isSchedulerOpen && (
        <SchedulerModal projectId={project.id} onClose={() => setIsSchedulerOpen(false)} />
      )}
    </div>
  );
};
...
const SchedulerModal = ({
  ...
  const handleTaskChange = (
    index: number,
    field: keyof SchedulerTaskInput,
    value: any
  ) => {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      const result = await apiClient.post(`/v1/projects/${projectId}/schedule`, {
        tasks: validTasks,
      });
            ...
            <div key={index} className="p-3 bg-gray-700 rounded-lg space-y-2">
              ...
              <div className="flex gap-2">
                ...
                <input
                  ...
                  value={task.dueDate || ''}
                  onChange={(e) =>
                    handleTaskChange(index, 'dueDate', e.target.value || null)
                  }
